...
import layoutStyle from './GridLayout.css';
...
class GridLayout extends React.Component<Props, State> {
  ...
  getGridItems() {
    ...
    const gridItems = React.Children.map(children, (child) => {
      ...
      return (
        <div className={layoutStyle['grid-layout-container']} style={itemStyle}>
          {child}
        </div>
      );
    });
  ...
  render() {
    ...
    return (
      <div className={layoutStyle['grid-layout']} style={containerStyle}>
        {this.getGridItems()}
      </div>
    );
  }
